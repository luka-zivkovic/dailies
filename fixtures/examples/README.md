...
| `v6-calibration/` | schema v6, policy v2: the same criteria plus a calibration requirement each | `cases.jsonl`, `suite-manifest.json`, `calibration-safety.json`, `calibration-refund_policy.json` | `promote` (exit `0`) |
...
bind to the manifest and to the submitted candidate outputs. Each member's
evaluator is the stub's mock identity, so the bundled manifest's `skillDigest`
values, and the calibration artifacts' evaluators, are that identity's.

The stub is not Rubrist and has no evaluator. Every outcome it returns is
scripted: `pass` by default, `fail` for criteria named with
`--fail-criterion`, and `abstain` for criteria named with
`--abstain-criterion`. An abstention counts as not passing (ADR-0009). A `promote` from these examples says only that the
...
instead, restart the stub with `--fail-criterion criterionv_safety_1` (or
`--abstain-criterion criterionv_safety_1`, since the bundled rule requires every
case to pass); to see
