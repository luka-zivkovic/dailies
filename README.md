...
> The real set, on the real rig, before doors open.

Soundcheck is the release layer of an AI trust ecosystem: safe shipping of AI changes (prompts, models, configs) via shadow evaluation and judged promotion. See [PLAN.md](./PLAN.md) for the product thesis and roadmap.

**What exists today (v0 wedge):** the `soundcheck` CLI ("shadow-run" mode) that runs a candidate AI change against historical inputs, judges each result against your production baseline, and emits a promote/block report. No serving-path changes required.
...
If *all* items error (candidate/judge unreachable, every call timing out, …), the run is a systemic failure rather than a verdict on the candidate, so `soundcheck` exits `2` instead of `1`. The report is still written; `totals.allErrored` is `true`.
...
`soundcheck --config shadow.config.json`. Relative paths are resolved against the config file's directory.
