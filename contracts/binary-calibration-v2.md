...
- an item that never reached the provider is `notAttempted`, and private
  ledger records carry the shared item result;
...
`result` (the shared item result of ADR-0014 section 6: `{ "state":
"outcome", "outcome": "pass" | "fail" | "abstain" }`, `{ "state": "failure",
"failureKind": ... }`, or `{ "state": "not_attempted" }`), `attemptState`
(`not_started`, `started`, or `terminal`), a closed `providerObservation`
(`provider`, `observedModel`, `observedVersion`, `systemFingerprint`,
`upstreamProvider`), exact
...
The ledger uses `rubrist-canonical-json/v1`. A `not_attempted` result pairs
with `not_started`; outcomes and known failures pair with `terminal`; and an
`outcome_unknown` failure may remain `started` because physical completion
...
are nullable strings under the same cap. A failure's `failureKind` uses the
shared closed taxonomy. `upstreamProvider` is non-null only when the
requested provider is `openrouter`. All objects are closed, all counts/indexes are
...
A `not_attempted`/`not_started` record has zero calls and no observed
model, version, fingerprint, or upstream. Outcomes have at least one call.
`outcome_unknown`/`started` has at least one call. Any non-null observed
...
`skillDigest` is SHA-256 over canonical JSON of `evaluator.identity`, and
`requestedBindingDigest` is SHA-256 over canonical JSON of
`evaluator.identity.executionBinding`; the verifier recomputes both. The
...
sum exactly to `errored`; they use the shared failure taxonomy, including
`provider_rejected_request`. `outcome_unknown` means a call may have happened but
...
that served an observation and is `null` for every other provider; it
separates groups but never changes identity strength.
...
`{provider,observedModel,observedVersion,systemFingerprint,upstreamProvider,identityStrength}`;
...
outcome (`classified + abstained = planned`) and `errored = notAttempted = 0`.
...
ADR-0014 section 4 adds a capability re-check before the authorization
exposure event. Its probes use a fixed, non-sensitive input, never a sealed
item, and a re-check that no longer holds stops the run before any exposure.
