...
// Object members are defined as own properties, even for a __proto__ key, as JSON.parse does.
function defineMember(parent: unknown, key: string, value: unknown): void {
...
function addValue(parent: unknown, key: string, value: unknown): void {
  ...
  else defineMember(parent, key, value);
}
...
function applyMutation(artifact: Record<string, unknown>, mutation: Mutation): void {
  ...
  else defineMember(parent, key, mutation.value);
}
...
describe('vendored Rubrist binary calibration v2 contract (Dailies ADR-0008)', () => {
  ...
  it('accepts all four exact canonical transport fixtures with complete expected identity', () => {
    ...
    expect(Object.keys(vectors.expectedIdentityByFixture)).toHaveLength(4);
