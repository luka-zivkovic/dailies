...
const canonicalDecimalSchema = z.string().max(32).regex(CANONICAL_DECIMAL_PATTERN);
...
const requestedModelBindingSchema = z.object({
  provider: publicStringSchema,
  modelId: publicStringSchema,
  modelVersion: publicStringSchema,
  temperatureDecimal: canonicalDecimalSchema,
  topPDecimal: canonicalDecimalSchema.nullable(),
  endpointKind: z.enum(['managed', 'custom']),
  baseUrlDigest: digestSchema.nullable(),
...
const outcomesSchema = z.object({
  ...
  errors: z.array(z.object({ code: errorCodeSchema, count: positiveCountSchema }).strict()).max(9),
}).strict();
...
function validatePublicJsonDomain(value: unknown, depth = 0): void {
  ...
  if (Array.isArray(value)) {
    ...
    for (const entry of value) validatePublicJsonDomain(entry, depth + 1);
    return;
  }
  if (typeof value === 'object') {
    ...
    for (const [key, entry] of entries) {
      ...
      validatePublicJsonDomain(entry, depth + 1);
    }
    return;
  }
...
function requestedBindingDigest(artifact: BinaryCalibrationArtifact): string {
...
function verifyTrial(trial: BinaryCalibrationTrial, artifact: BinaryCalibrationArtifact): void {
  ...
  const sum = (counts: typeof outcomes.byTruth.pass) =>
    counts.classified + counts.abstained + counts.errored + counts.unevaluated;
  if (outcomes.planned !== artifact.truth.itemCount ||
    outcomes.classified + outcomes.abstained + outcomes.errored + outcomes.unevaluated !== outcomes.planned ||
    ...
    outcomes.byTruth.pass.unevaluated + outcomes.byTruth.fail.unevaluated !== outcomes.unevaluated) {
...
export function verifyBinaryCalibrationArtifact(
  ...
  if (!parsed.success) {
    throw new BinaryCalibrationIntegrityError(
      'invalid_structure',
      `binary calibration artifact does not match v1 contract: ${parsed.error.message}`,
      { cause: parsed.error },
    );
  }
  ...
  if ((binding.endpointKind === 'custom') !== (binding.baseUrlDigest !== null)) {
    throw new BinaryCalibrationIntegrityError(
      'semantic_mismatch',
      'custom requested model bindings require exactly one baseUrlDigest',
    );
  }
  if (compareExactRationals(parseCanonicalDecimalRational(binding.temperatureDecimal), { numerator: 2n, denominator: 1n }) > 0) {
    throw new BinaryCalibrationIntegrityError('semantic_mismatch', 'temperatureDecimal is outside its allowed range');
  }
  if (binding.topPDecimal !== null &&
    compareExactRationals(parseCanonicalDecimalRational(binding.topPDecimal), { numerator: 1n, denominator: 1n }) > 0) {
    throw new BinaryCalibrationIntegrityError('semantic_mismatch', 'topPDecimal is outside its allowed range');
  }
  if (binding.requestedBindingDigest !== requestedBindingDigest(artifact)) {
    throw new BinaryCalibrationIntegrityError('digest_mismatch', 'requestedBindingDigest mismatch');
  }
